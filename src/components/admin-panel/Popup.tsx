...
const Popup = ({ setOpenPopup, setUpdateTable }: PropsType) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    axios
      ...
      .catch((err) => console.log("popup-err", err))
      .finally(() => {
    ...
    <div className="fixed top-0 left-0 w-full h-screen bg-[#00000070] grid place-items-center ">
      <div className="bg-white w-[700px] py-8 rounded-lg text-center relative ">
        ...
        <form onSubmit={handleSubmit} className="mt-6 w-fit space-y-4 mx-auto">
          <input
            className="border block border-gray-500 outline-none py-2 px-4 round-lg w-fit "
            type="text"
            placeholder="Name"
          ...
          <div className="flex w-full">
            <button className="bg-[#c77848] block text-white px-8 py-2 rounded-lg self-center ">
              Save
            </button>
          </div>
        </form>
